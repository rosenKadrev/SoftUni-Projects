function search() {

    let townsLi = document.getElementsByTagName('li');
    let searchLetters = document.getElementById('searchText').value;
    let matchCounter = 0;

    for (const currentLi of townsLi) {
        currentLi.style.fontWeight = 'normal';
        currentLi.style.textDecoration = 'none';
        if (currentLi.textContent.includes(searchLetters)) {
            matchCounter++;
            currentLi.style.fontWeight = 'bold';
            currentLi.style.textDecoration = 'underline';
        }
    }
    document.getElementById('result').textContent = `${matchCounter} matches found`;

}



function search() {

    let listElements = document.querySelectorAll('ul#towns li');
    let resultElement = document.getElementById('result')
    let searchText = document.getElementById('searchText').value;
    let matches = 0;

    for (const element of listElements) {
        element.style.fontWeight = 'normal';
        element.style.textDecoration = 'none';
    }

    for (let element of listElements) {
        let text = element.textContent;

        if (text.match(searchText)) {
            matches++;
            element.style.fontWeight = 'bold';
            element.style.textDecoration = 'underline';
        }
    }
    resultElement.textContent = `${matches} matches found`;

}